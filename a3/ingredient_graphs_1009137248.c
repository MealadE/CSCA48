    ...
    if (k > 1) {
        for (int col = 0; col < MAT_SIZE; col++) {
            if (AdjMat[index][col] > 0) {
                ...
                h = related_k_dist(h, ingredients[col], k - 1, dist);
            }
        }
    }
    return h;
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    intNode *avoid1 = related_k_dist(NULL, avoid, k_avoid, 0);

    intNode *p = NULL;
    intNode *current = source;
    while(current != NULL){
        if(searchInt(avoid1, current->x) == 0 && current->x != ingredient_index(avoid))
            p = insertInt(p, current->x);
        current = current->next;
    }

    source = deleteList(source);
    avoid1 = deleteList(avoid1);
    ...
    ******/
    ...
    for(int i = 0; strcmp(recipe[i], "") != 0; i++){
        ...
        for (int j = 0; j < MAT_SIZE; j++){
            ing_list[j] += AdjMat[ingredient_index(recipe[i])][j];
        }
    }
    ...
    if(highest != 0){
        strcpy(recipe[replace],ingredients[position]);
    }
    else{
        strcpy(recipe[replace],"");
    }
