...
BST_Node *find_successor(BST_Node *right_child_node)
{
     ...
     ****/
     if (right_child_node==NULL){
         ...
         BST_Node *left_node=right_child_node->left;
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    if (root->bar>bar || (root->bar==bar && root->index>index)) {
        ...
        if (root->left == NULL) {
            ...
            BST_Node *left = find_successor(root->right);
            // Copy the inorder successor's content to this node
            root->index = left->index;
            root->freq = left->freq;
            root->bar = left->bar;

            // Delete the inorder successor
            root->right = BST_delete(root->right, left->bar, left->index);
        }
    }
    return root;
}
...
void switchnode(BST_Node *root, int max, int min){
    ...
    root->bar = (max + min) - root->bar;
    root->key=10*(root->bar)+root->index;
}
void reverse(BST_Node *root, int max, int min){

    if(root == NULL) return;

    // Using post-order traversal for reversal //
    reverse(root->left, max, min);
    reverse(root->right, max, min);
    switchnode(root, max, min);
}
...
int semiindex(double freq, int semitones){
    for(int i= 0;i<= 99; i++){
        if(note_freq[i] == freq){
            return i+semitones;
        }
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    double new_time = root->index+time_shift;
    int newindex = semiindex(root->freq, semitones);
    BST_Node *new = NULL;
    if(newindex <= 99 && newindex >= 0 && new_time>=0.0 && new_time <1 && new_time != -1-time_shift){
        new = newBST_Node(note_freq[newindex], root->bar, new_time);
        BST_insert(root,new);
